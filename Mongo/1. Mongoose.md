...
```
note the example of data validation in age, while for numbers we have min and max properties for string we have minLength and maxLength properites like so
```js
const UserSchema=new mongoose.Schema({
	name:{
		type:String,
...
so just do findById make your changes and save
#### read
```js 
user.findById("1");
user.findOne({
...
```js
user.deleteMany({});
user.deleteOne({username:"chait8126@gmail.com"});//returns the count of the deleted, {deletedCount:1}
```
#### LINQ style queries
```js
User.where()
